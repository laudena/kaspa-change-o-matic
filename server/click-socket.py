...
def read_button():
    global LAST_PRESS_TIME, BUTTON_RELEASED
    try:
        ...
        if current_state and BUTTON_RELEASED and (time.time() - LAST_PRESS_TIME > DEBOUNCE_INTERVAL):
            ...
            BUTTON_RELEASED = True  # Button is released
        ...
        if prev_time_low is not None:
            ...
            if SIGNAL_MIN_DURATION <= signal_duration <= SIGNAL_MAX_DURATION:
                if not measuring:
                    ...
                    print(f"Detecting first signal")
                else:
                    signal_count += 1  # Count additional valid signals
                    print(f"Detected {signal_count} signals")
...
async def handle_coin_received(amount):
    print(f"Detected {signal_count} signals within {TIME_WINDOW}ms")
    global shared_data
    shared_data["collected_amount"] += amount
    print(f"1. Coin received: {amount} AUD. Total: {shared_data['collected_amount']} AUD.")
...
async def handle_scan_user_address():
    print(f"2. After Confirming amount: {shared_data['collected_amount']} AUD")
    async def wrapper():
...
async def handle_scan_result(success, qr_code):
    print(f"[{time.time()}] 3. After scan complete/failed")

    if success:
        print(f"[{time.time()}] QR Code Scan Successful. Address: {qr_code}")
        shared_data["recipient_address"] = qr_code
    else:
        print(f"[{time.time()}] QR Code Scan Failed. keep trying until timeout")
    ...
    print(f"[{time.time()}] UI update sent.")
    if shared_data['collected_amount'] <= 0:
        print(f"[{time.time()}] Failed to transmit due to missing amount: {shared_data['collected_amount']}")
        await send_current_screen("error-page", f"${shared_data['collected_amount']}... Well, that's not much, /nbut you can add coins at any time, /nhow about now? ")
    elif shared_data['recipient_address'] == "":
        print(f"[{time.time()}] Failed to transmit due to missing recipient address: { shared_data['recipient_address']}.")
        await send_current_screen("error-page", f"The recipient address is wrong or missing. Look:  { shared_data['recipient_address']}.")
    elif not success:
        print(f"[{time.time()}] Failed to transmit due to general qr-code reading error, most likely - timeout.")
        ...
        print(f"[{time.time()}] 4. Requesting transaction..")
        submission_result = await run_kaspa_transaction()
        if submission_result:
            print(f"[{time.time()}] 5. Transaction complete, resetting variables.")
    ...
    while True:
        ...
        if output:
            try:
                ...
                print(f"[UNKNOWN] {output}")  # Handle non-JSON outputs
...
def coin_signal_callback(chip, gpio, level, tick):
    ...
    # Check if the signal is going HIGH (rising edge)
        ...
        if signal_duration >= SINGLE_SIGNAL_DURATION * 0.7:
            print(f"Signal detected, duration: {signal_duration*1000:.2f}ms")

            # If no previous signal was counted
            if signal_count == 0:
                signal_count += 1
                last_signal_time = signal_end_time
                print("Single signal detected!")
    ...
    print(f"level: {level} Signal Start: {signal_start_time}, Signal End: {signal_end_time}, Signal Count: {signal_count}")
...
async def send_periodic_updates():
    while True:
        ...
        print ("exchange-update:", " kaspa_price:", kaspa_price, " usd_to_aud:", usd_to_aud)
...
async def client_handler(websocket, path=None):
    global current_screen
    print("WebSocket connected. current screen:" + current_screen, end="", flush=True)
    connected_clients.add(websocket)
    try:
        # Log active connections immediately
        print(f"Active connections: {len(connected_clients)}", flush=True)
        ...
        print("WebSocket disconnected. ", end="", flush=True)
...
async def run_transaction_retries():
    ...
    async def read_output(stream):
        while True:
            ...
            if not line:
                break
            print(f"[MONITOR] {line.decode().strip()}")
