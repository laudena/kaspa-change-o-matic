 ...
 * @returns {{address: Address, tokens: any, networkId: (NetworkId), encoding: (Encoding), amount: string}}
 */
function parseArgs(options = {
    ...
    const {
        ...
        args, options: {
            ...
            amount: { type: 'string' } // Added support for --amount
    ...
    if (values.help) {
        ...
        --encoding <borsh|json> 
        --amount <amount_in_kaspa> ${options.additionalHelpOutput}`);
        process.exit(0);
    }
    ...
    return {
        ...
        networkId,
        encoding,
        amount: values.amount ?? null // Pass the amount if provided
    };
}
